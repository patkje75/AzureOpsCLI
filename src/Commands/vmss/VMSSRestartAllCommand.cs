...
{
    public class VMSSRestartAllCommand : AsyncCommand
    {
        private readonly IVMSSService _vmssService;

        public VMSSRestartAllCommand(IVMSSService vmssService)
        {
            ...
            new SpinnerColumn(),
                ...
                .StartAsync(async ctx =>
                {
                    var tasks = selectedVMs.Select(vmName =>
                    {
                        ...
                        return _vmssService.RestartVMSSAsync(vmExtended.VMSS)
                            .ContinueWith(t =>
                            {
                                if (t.Result.Success)
                                {
