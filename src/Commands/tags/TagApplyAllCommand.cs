...
{
    public class TagApplyAllCommand : AsyncCommand
    {
        private readonly ITagService _tagService;

        public TagApplyAllCommand(ITagService tagService)
        {
            ...
            try
            {
                ...
                var selectedResources = AnsiConsole.Prompt(
                    new MultiSelectionPrompt<string>()
                        ...
                        .AddChoices(resources.Select(r => $"{r.ResourceName} ({r.ResourceType.Split('/').LastOrDefault()}) - {r.SubscriptionName}"))
                );
