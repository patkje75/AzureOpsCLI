...
{
    public class TagRemoveAllCommand : AsyncCommand
    {
        private readonly ITagService _tagService;

        public TagRemoveAllCommand(ITagService tagService)
        {
        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            try
            {
                ...
                {
                    AnsiConsole.MarkupLine("[red]No resources with tags found.[/]");
                    return -1;
                }
                ...
                var selectedResources = AnsiConsole.Prompt(
                    new MultiSelectionPrompt<string>()
                        ...
                        .AddChoices(resourcesWithTags.Select(r => $"{r.ResourceName} ({r.ResourceType.Split('/').LastOrDefault()}) - {r.SubscriptionName} [{r.Tags.Count} tags]"))
                );
