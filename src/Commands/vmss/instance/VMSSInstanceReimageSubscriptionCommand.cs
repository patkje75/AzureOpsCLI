...
{
    public class VMSSInstanceReimageSubscriptionCommand : AsyncCommand
    {
        private readonly IVMSSVMService _vmssvmService;
        private readonly ISubscritionService _subscriptionService;

        public VMSSInstanceReimageSubscriptionCommand(IVMSSVMService vmssvmService, ISubscritionService subscritionService)
        {
        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            ...
            var runningVMSSInstances = await _vmssvmService.FetchVMSSInstancesInSubscriptionAsync(subscriptionId);

            if (!runningVMSSInstances.Any())
            {
                AnsiConsole.MarkupLine("[red]No Virtual Machine Scale Set instances found in the selected subscription.[/]");
                return 0;
            }
            ...
            var selectedVMs = AnsiConsole.Prompt(
                new MultiSelectionPrompt<string>()
                    ...
                    .InstructionsText("[grey](Press [blue]<space>[/] to toggle a Scale Set instance, [green]<enter>[/] to reimage selected Scale Set instance)[/]")
                    .AddChoices(vmNames));
