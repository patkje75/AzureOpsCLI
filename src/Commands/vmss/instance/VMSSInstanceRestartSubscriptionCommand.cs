...
{
    public class VMSSInstanceRestartSubscriptionCommand : AsyncCommand
    {
        private readonly IVMSSVMService _vmssvmService;
        private readonly ISubscritionService _subscriptionService;

        public VMSSInstanceRestartSubscriptionCommand(IVMSSVMService vmssvmService, ISubscritionService subscritionService)
        {
        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            ...
            var selectedVMs = AnsiConsole.Prompt(
                new MultiSelectionPrompt<string>()
                    ...
                    .InstructionsText("[grey](Press [blue]<space>[/] to toggle a Scale Set instance, [green]<enter>[/] to restart selected Scale Set instance)[/]")
                    .AddChoices(vmNames));
                    ...
                    var tasks = selectedVMs.Select(vmName =>
                    {
                        ...
                        return _vmssvmService.RestartVMSSInstanceAsync(vmExtended.VMSSVm)
                            .ContinueWith(t =>
                            {
                                ...
                                {
                                    task.Description = $"[red]Failed[/] to restart [blue]{vmExtended.VMSSVm.Data.Name}[/]: {t.Result.Message}";
                                    task.Increment(100);
                                }
                            });
                    }).ToList();
