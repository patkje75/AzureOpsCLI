...
{
    public class VMSSInstanceStartSubscriptionCommand : AsyncCommand
    {
        private readonly IVMSSVMService _vmssvmService;
        private readonly ISubscritionService _subscriptionService;

        public VMSSInstanceStartSubscriptionCommand(IVMSSVMService vmssvmService, ISubscritionService subscritionService)
        {
        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            ...
            var deallocatedVMSSs = await _vmssvmService.FetchStoppedVMSSInstancesInSubscriptionAsync(subscriptionId);

            if (!deallocatedVMSSs.Any())
            {
                AnsiConsole.MarkupLine("[red]No stopped Virtual Machine Scale Set instances found in the selected subscription.[/]");
                return 0;
            }
            ...
            var selectedVMs = AnsiConsole.Prompt(
                new MultiSelectionPrompt<string>()
                    ...
                    .InstructionsText("[grey](Press [blue]<space>[/] to toggle a Scale Set instance, [green]<enter>[/] to start selected Scale Set instance)[/]")
                    .AddChoices(vmNames));
