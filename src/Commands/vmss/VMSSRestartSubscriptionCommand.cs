...
{
    public class VMSSRestartSubscriptionCommand : AsyncCommand
    {
        private readonly IVMSSService _vmssService;
        private readonly ISubscritionService _subscriptionService;

        public VMSSRestartSubscriptionCommand(IVMSSService vmssService, ISubscritionService subscritionService)
        {
        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            ...
            var runningVMSSs = await _vmssService.FetchRunningVMSSInSubscriptionAsync(subscriptionId);

            if (!runningVMSSs.Any())
            {
                AnsiConsole.MarkupLine("[red]No running Virtual Machine Scale Sets found in the selected subscription.[/]");
                return 0;
            }
            ...
            var selectedVMs = AnsiConsole.Prompt(
                new MultiSelectionPrompt<string>()
                    .Title("Select Scale Set to restart:")
                    .NotRequired()
                    .PageSize(10)
                    .MoreChoicesText("[grey](Move up and down to reveal more Virtual Machine Scales Sets)[/]")
                    .InstructionsText("[grey](Press [blue]<space>[/] to toggle a Scale Set, [green]<enter>[/] to restart selected Scale Set)[/]")
                    .AddChoices(vmNames));
            ...
            new SpinnerColumn(),
                ...
                .StartAsync(async ctx =>
                {
                    var tasks = selectedVMs.Select(vmName =>
                    {
                        ...
                        var task = ctx.AddTask($"Restarting {vmExtended.VMSS.Data.Name}");
                        ...
                        return _vmssService.StopVMSSAsync(vmExtended.VMSS)
                            .ContinueWith(t =>
                            {
                                ...
                                {
                                    task.Description = $"[red]Failed[/] to restart [blue]{vmExtended.VMSS.Data.Name}[/]: {t.Result.Message}";
                                    task.Increment(100);
                                }
                            });
                    }).ToList();
