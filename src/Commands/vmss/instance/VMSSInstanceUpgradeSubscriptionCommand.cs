        ...
        public override async Task<int> ExecuteAsync(CommandContext context)
        {
            ...
            var selectedVMs = AnsiConsole.Prompt(
                new MultiSelectionPrompt<string>()
                    .Title("Select Scale Set instance to upgrade to the latest model:")
                    .NotRequired()
                    .PageSize(10)
                    .MoreChoicesText("[grey](Move up and down to reveal more Scale Set instances)[/]")
                    .InstructionsText("[grey](Press [blue]<space>[/] to toggle a Scale Set instance, [green]<enter>[/] to upgrade selected Scale Set instance)[/]")
                    .AddChoices(vmNames));
                    ...
                    var tasks = selectedVMs.Select(vmName =>
                    {
                        ...
                        return _vmssvmService.UpgradeVMSSInstanceToLatestModelAsync(vmExtended.VMSSVm, vmExtended.VMSS)
                            .ContinueWith(t =>
                            {
                                ...
                                {
                                    task.Description = $"[red]Failed[/] to upgrade [blue]{vmExtended.VMSSVm.Data.Name}[/]: {t.Result.Message}";
                                    task.Increment(100);
                                }
                            });
                    }).ToList();
