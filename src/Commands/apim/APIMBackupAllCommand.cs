...
{
    public class APIMBackupAllCommand : AsyncCommand
    {
        ...
        public APIMBackupAllCommand(IAPIManagementService apiManagementService, IStorageService storageService, ISubscritionService subscriptionService)
        {
        ...
        {
            var apimServices = await _apiManagementService.FetchAllAPIMAsync();
            if (!apimServices.Any())
            {
